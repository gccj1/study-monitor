...
public class WebSocketConfig {
    ...
    private static CliDetailMapper detailMapper;

    @Resource
    public void setDetailMapper(CliDetailMapper detailMapper) {
        WebSocketConfig.detailMapper = detailMapper;
    }
    ...
    public void setSshMapper(SshMapper sshMapper) {
        WebSocketConfig.sshMapper = sshMapper;
    }
    ...
    public void onOpen(Session session,
        ...
        SshInfo ssh = sshMapper.selectById(clientId);
        if(detail!=null) connectionIp= Optional.ofNullable(ssh.getIp()).orElse(detail.getIp());
        else connectionIp=ssh.getIp();
//        if(detail == null || ssh == null) {
//            session.close(new CloseReason(CloseReason.CloseCodes.CANNOT_ACCEPT, "无法识别此主机"));
//            return;
//        }

        if(this.createSshConnection(session, ssh, connectionIp)) {
            log.info("主机 {} 的SSH连接已创建",connectionIp);
        }
    }
    ...
    private boolean createSshConnection(Session session, SshInfo ssh, String ip) throws IOException{
        try {
            JSch jSch = new JSch();
