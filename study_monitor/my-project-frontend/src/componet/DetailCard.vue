...
import {cpuNameToImage, fitByUnit, osNameToIcon, percentageToStatus} from "@/utils";
import {ElMessage, ElMessageBox} from "element-plus";
import {Connection, Delete} from "@element-plus/icons-vue";
import RuntimeHistory from "@/componet/RuntimeHistory.vue";
...
function deleteClient() {
  ElMessageBox.confirm('删除此主机后所有统计数据都将丢失，您确定要这样做吗？', '删除主机', {
    ...
    get(`/api/monitor/delete?id=${props.id}`, () => {
      emits('delete')
      props.update()
...
setInterval(() => {
  if(props.id !== -1 && details.runtime) {
    get(`/api/monitor/runtime-now?id=${props.id}`, data => {
      if(data.timestamp === details.runtime.history[details.runtime.history.length - 9].timestamp){
        location.reload()
      }
     else if(details.runtime.history.length >= 360)
        details.runtime.history.splice(0, 1)
      details.runtime.history.push(data)
    })
  }
}, 3000)

const now = computed(() => details.runtime.history[details.runtime.history.length - 1])

const init = id => {
  if(id !== -1) {
    details.base = {}
    details.runtime = { history: [] }
    get(`/api/monitor/details?id=${id}`, data => Object.assign(details.base, data))
     get(`/api/monitor/runtime-history?id=${id}`, data => Object.assign(details.runtime, data))
  }
}
    ...
    <div class="client-details" v-loading="Object.keys(details.base).length === 0">
      <div v-if="Object.keys(details.base).length">
        <div style="display: flex;justify-content: space-between">
          ...
          <div>
            <el-button :icon="Connection" type="primary"
                       @click="emits('terminal', id, details.base.ip)" plain text>SSH远程连接</el-button>
        ...
        <div class="details-list">
          ...
          <div style="display: flex">
            <span>处理器</span>
            <el-image style="height: 20px;margin-right:5px; "
        ...
        <el-divider style="margin: 10px 0"/>
        <div v-if="details.base.online" v-loading="!details.runtime.history.length"
             style="min-height: 200px">
          <div style="display: flex" v-if="details.runtime.history.length">
          ...
          <runtime-history style="margin-top: 20px" :data="details.runtime.history" />
        </div>
